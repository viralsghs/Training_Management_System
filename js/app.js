...
const Shell = {
  ...
  renderTopbar(title) {
    ...
    const pageTitle = title || labels[currentPage] || 'Training System';
...
const DEMO = {
  isDemoMode() {
    // CONFIG.isDemo is set in config.js based on whether API_URL still has placeholder.
    // Also treat DEMO-prefixed user IDs as demo mode (demo accounts from login page).
    const user = Session.get();
    return CONFIG.isDemo || (user && user.id && user.id.startsWith('DEMO'));
  },
